...
<head>
    <meta charset="UTF-8">
    <title>Bon de Commande - CMD-{{ str_pad($commande->id, 6, '0', STR_PAD_LEFT) }}</title>
    <style>
...
<body>
    ...
    <div class="box">
        ...
        <div><strong>Numéro:</strong> {{ $commande->fournisseur->numero }}</div>
        <div><strong>Adresse:</strong> {{ $commande->fournisseur->adresse }}</div>
        <div><strong>Date commande:</strong> {{ $commande->date_commande->format('d/m/Y') }}</div>
        <div><strong>Livraison prévue:</strong> {{ $commande->date_livraison_prevue->format('d/m/Y') }}</div>
    </div>
    ...
    <p class="right total">Montant total: {{ number_format($commande->montant_total, 0, ',', ' ') }} FCFA</p>
</body>
</html>
